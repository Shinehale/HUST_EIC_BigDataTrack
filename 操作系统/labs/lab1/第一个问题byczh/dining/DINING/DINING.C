...
void RenderOffscreen(HDC hDestDC)
{
	...
	/* Draw the chopsticks */
	dDeltaAng = 360 / PHILOSOPHERS;                 //筷子间相差的角度
	for (pos = 0; pos < PHILOSOPHERS; pos++)	//FIXIT
	{
		/* Draw the chopsticks */
		AngRad = (pos * dDeltaAng)/57.29577951;      //转换成弧度  
		dx = CenterX + (sin(AngRad)*60);             //获得筷子的第一个点
		dy = CenterY - (cos(AngRad)*60);
		MoveToEx(hdc, (int)dx, (int)dy, NULL);       
		dx = CenterX + (sin(AngRad)*85);             //获得筷子的第二个点
		dy = CenterY - (cos(AngRad)*85);
		LineTo(hdc, (int)dx, (int)dy);

		//Draw the plate                                
		AngRad = ((pos * dDeltaAng+dDeltaAng / 2))/57.29577951; //the angle of plates
		dx = CenterX + (sin(AngRad) * 72);                      //the center of each plate
	...
	for(pos = 0; pos < PHILOSOPHERS; pos++)
	{
		...
		//Draw the left arm
		if (gChopstickState[pos] == pos)   //左边筷子的状态的等于哲学家的号码
		{
		...
		//Draw the right arm
		p1 = pos + 1;              //如果筷子的编号是4，说明右边的筷子是第一个筷子了
		if (p1 == PHILOSOPHERS) 
			p1 = 0;
		if (gChopstickState[p1] == pos) //看右边的筷子的编号是否跟哲学家的编号一致
		{
			MoveToEx(hdc, (int)px, (int)py, NULL);
	...
	LPSTR lpCmdLine, int nCmdShow)
{
	...
	while (GetMessage(&msg, NULL, 0, 0))  //处理进程调度
	{
