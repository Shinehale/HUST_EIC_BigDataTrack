...
/*long*/LRESULT CALLBACK WndProc(HWND, UINT, WPARAM, LPARAM);
...
{
	PAINTSTRUCT ps;//包含画应用程序拥有的客户区域的信息
	HDC hdc;
	...
	switch (message) 
	{
		...
		case WM_FORCE_REPAINT:
			{
				...
				InvalidateRect(hWndMain, NULL, TRUE);//更新客户窗口区域,重绘
		...
		case WM_CLOSE:
			return DefWindowProc(hWndMain, message, wParam, lParam);//调用缺省窗口过程处理消息

		case WM_DESTROY:
			PostQuitMessage(0);//线程中止
			break;
...
/*BOOL CreateOffscreen()//创建什么位图?
{
	HWND hwndScreen = GetDesktopWindow();//获取桌面窗口句柄
	HDC hdc	= GetDC(hwndScreen);//获取客户区域设备上下文

	int	nWidth	= GetSystemMetrics( SM_CXSCREEN );
	int	nHeight	= GetSystemMetrics( SM_CYSCREEN );//获取显示窗口尺寸

	hbmpOffscreen = CreateCompatibleBitmap( hdc, nWidth, nHeight );//创建与设备兼容的位图
...
void RenderOffscreen(HDC hDestDC)
{
	...
	HBITMAP hOldBitmap	= SelectObject(hdc, hbmpOffscreen);//返回什么位图?将对象选入确定的设备上下文中
	RECT rect;
	HPEN hPen;
...
//	DeleteObject(SelectObject(hdc, hPen));//?为什么要删除
...
//	SelectObject(hdc, hOldBitmap);//选择得什么位图
	...
	LPSTR lpCmdLine, int nCmdShow)
{
	...
	while (GetMessage(&msg, NULL, 0, 0)) //从线程队列中获取消息
	{
		TranslateMessage(&msg);//将键盘消息转换为字符消息
	 	DispatchMessage(&msg); //将消息分派到窗口过程中
	}
	// Clear the table
	for (i = 0; i < PHILOSOPHERS; i++)
		CloseHandle(gchopStick[i]); //关闭互斥体句柄,并不中止线程
  	return (msg.wParam);
}
